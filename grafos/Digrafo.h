...
~Digrafo(); // destructor
int getVertices() const; // obtener eL numéro de vertices
void setVertices(int vertices); // estabLecer eL numéro de vertices
int getAristas() const; // obtener ei numéro de aristas
...
for (it = otro.adyacentes[i].begin(); it != otro.adyacentes[i].end(); it++) adyacentes[i].push_back(*it);
}
}
}
...
void Digrafo::imprimir() {
	...
	cout << i << " : ";
	list<int>::iterator it;
	for (it = adyacentes[i].begin();
 		it != adyacentes[i].end(); it++) cout << *it << ' '; cout << endl;
}
}
#endif // !DIGRAFO_H
