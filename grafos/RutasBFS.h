...
private:
	...
	int vertices; // numéro de vertices del grafo origen bool *Marcado; // marcado[v] = hay un camino entre s-v?
	bool *marcado;
	int *aristaHacia; // aristaHacia[v] = ultima arista en La ruta s-v mas corta int *distanciaHacia; // distanciaHaciafv] = Longitud de La ruta s-v mas corta void bfs(const Digrafo &G, int s); // aLgoritmo BFS recursivo public :
	int *distanciaHacia;
	void bfs(const Digrafo &G, int s);

public:
	RutaBFS(const Digrafo &G, int s); // constructor RutaBFS(const RutaBFS &otra); // copy constructor ~RutaBFS(); // destructor
